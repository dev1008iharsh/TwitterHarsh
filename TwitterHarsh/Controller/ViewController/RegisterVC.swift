...
class RegisterVC: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
         vigationController?.navigationBar.isHidden = false
    }
    ...
    private func configCombineView(){
        ...
        ed.Enjoy HarshTwitter social media application created by Harsh Darji", view: strongSelf) { action in
    ...
    @objc func ter.validateAuthForm()
    }
    ...
    @IBAction func btnRegister(_ sender: UIButton) {
        ...
        if !(Utils.isValidEmail(txtEmail.text ?? "")){
            ...
            Utils.showAlert(title: "Invalid password", message: "Minimum 8 characters are required in the password including atleast one special character, one capital letter, one small letter, one upper case, one lower case, and one digit", view:  
/*
