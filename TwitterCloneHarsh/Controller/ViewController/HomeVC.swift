...
class HomeVC: UIViewController {
    ...
    private lazy var stackView : UIStackView = {
        let stackView = UIStackView(arrangedSubviews: tabsBtn)
            ...
            for i in 0..<tabsBtn.count{
                UIView.animate(withDuration: 0.3, delay: 0,options: .curveEaseInOut) {[weak self] in
                    ...
                    if self?.tabSelected == 0{
                        self?.viewModelHome.fetchAllTweetsByAllUsers()
                        print("called selectedTab 00000")
                    }else{
                        self?.viewModelHome.fetchCurrentLoggedInUserTweets()
                        print("called selectedTab 11111")
                    }
    ...
    func configureCombineView() {
        ...
        .store(in: &subscriptionsHome)
        
        viewModelHome.$tweetsHome.sink { [weak self] _ in
    ...
    private func handleAuthentication(){
        ...
        if Auth.auth().currentUser == nil{
            ...
            guard let nextVC = storyBoard.instantiateViewController(withIdentifier: Constants.VC_LOGIN) as? LoginVC else { return }
            self.navigationController?.pushViewController(nextVC, animated: false)
            
        }else{
            if let id = Auth.auth().currentUser?.uid {
                Utils.myUserIDglobal = id
            }
